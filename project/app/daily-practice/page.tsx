...
import { ArrowLeft, Clock, Play, Pause, SkipForward, SkipBack, RotateCcw } from 'lucide-react';
...
export default function DailyPracticePage() {
  ...
  // Fetch audio and script for daily practice session
  ...
  // Timer progress
  const timerProgress = 1 - timeLeft / 600;
    ...
    return (
      <div className="min-h-screen flex flex-col relative overflow-hidden">
        <motion.div
          ...
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          style={{ background: 'linear-gradient(120deg, #e0eaff 0%, #c3f0fc 50%, #fcdffb 100%)' }}
        />
        <div className="max-w-lg w-full mx-auto py-10 px-4 flex-1 flex flex-col justify-center items-center relative z-10">
          ...
          >
            ...
            <p className="text-lg text-gray-600 dark:text-gray-300 mb-8">
              You've completed your Daily Practice meditation. Carry this clarity into your day.
            </p>
            <div className="flex gap-4 justify-center">
              <Button onClick={handleRestart} size="lg" className="bg-gradient-to-r from-blue-500 to-purple-500">
  ...
  return (
    <div className="min-h-screen flex flex-col relative overflow-hidden">
      ...
      >
        {/* Floating shapes */}
        <motion.div
          className="absolute top-10 left-10 w-32 h-32 rounded-full bg-blue-200 opacity-30"
          animate={{ y: [0, 20, 0], x: [0, 10, 0] }}
          transition={{ duration: 6, repeat: Infinity, ease: 'easeInOut' }}
        />
        <motion.div
          className="absolute bottom-10 right-10 w-40 h-40 rounded-full bg-purple-200 opacity-20"
          animate={{ y: [0, -20, 0], x: [0, -10, 0] }}
      ...
      <div className="absolute top-4 right-4 z-20">
        <div className="relative w-16 h-16 flex items-center justify-center">
          <svg className="absolute top-0 left-0" width="64" height="64">
            <circle cx="32" cy="32" r="28" stroke="#818cf8" strokeWidth="6" fill="none" opacity="0.2" />
            <circle
              cx="32" cy="32" r="28"
              stroke="#818cf8"
              strokeWidth="6"
          ...
          <span className="absolute text-blue-700 font-bold text-lg">{formatTime(timeLeft)}</span>
        </div>
      </div>
      ...
      <div className="max-w-lg w-full mx-auto py-10 px-4 flex-1 flex flex-col justify-center items-center relative z-10">
        ...
        >
          <motion.div
            className="h-16 w-16 flex items-center justify-center rounded-full bg-gradient-to-br from-blue-400 to-purple-500 mb-4 shadow-lg"
            animate={{ scale: [1, 1.08, 1] }}
            transition={{ duration: 2, repeat: Infinity, ease: 'easeInOut' }}
          >
            <Clock className="h-10 w-10 text-white" />
          </motion.div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2 text-center">Daily Practice</h1>
          <p className="text-lg text-blue-700 dark:text-blue-200 text-center">10-minute regular meditation</p>
        </motion.div>
            ...
            {scriptSteps.map((_, i) => (
              <span 
                ...
                className={`w-3 h-3 rounded-full transition-all duration-300 ${
                  ...
                  i < step ? 'bg-blue-400' : 'bg-blue-200'
                }`} 
              />
            ))}
          </div>
